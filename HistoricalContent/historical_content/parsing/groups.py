...
def _match_block(value: object, field: str, errors: list[str]) -> dict[str, list[str]]:
    ...
    return {
        ...
        "prefixes": _string_list(
            value.get("prefixes", []), f"{field}.prefixes", errors
        ),
...
def validate_group_config(payload: object) -> list[str]:
    ...
    for index, group in enumerate(groups):
        ...
        for topic in match["topics"]:
            ...
            if previous:
                errors.append(
                    f"Topic {topic!r} is assigned to both {previous!r} and {target!r}."
                )
        ...
        for prefix in match["prefixes"]:
            ...
            if previous:
                errors.append(
                    f"Prefix {prefix!r} is assigned to both {previous!r} and {target!r}."
                )
        ...
        for sub_index, subgroup in enumerate(subgroups):
            ...
            if not isinstance(sub_id, str) or not sub_id:
                ...
                errors.append(
                    f"Duplicate subgroup ID {sub_id!r} in group {group_id!r}."
                )
            ...
            if not isinstance(sub_label, str) or not sub_label:
                ...
                sub_label = sub_id
            sub_match = _match_block(
                subgroup.get("match"), f"{sub_field}.match", errors
            )
    ...
    if not isinstance(overrides, list):
        ...
        for index, override in enumerate(overrides):
            ...
            if not isinstance(override, dict):
                errors.append(f"{field} must be an object.")
                continue
            if not isinstance(override.get("filename"), str) or not override.get(
                "filename"
            ):
            ...
            if target_group not in group_ids:
                errors.append(
                    f"{field}.group refers to unknown group {target_group!r}."
                )
...
def load_group_config(path: str | Path = DEFAULT_GROUP_CONFIG_PATH) -> dict[str, Any]:
    ...
    try:
        ...
        raise GroupConfigError(
            f"Could not load voiceline groups from {config_path}: {exc}"
        ) from exc
    errors = validate_group_config(payload)
    if errors:
        raise GroupConfigError(
            "Invalid voiceline group configuration:\n- " + "\n- ".join(errors)
        )
...
def _compile_group_rules(config: dict[str, Any]) -> dict[str, Any]:
    ...
    prefixes: dict[str, list[tuple[str, tuple[str, ...], tuple[str, ...]]]] = {
        "voice": [],
        "ping": [],
    }
...
def sort_subject_topics(
    config: dict[str, Any], topics: dict[str, Any]
) -> dict[str, Any]:
    ...
    ordered = dict(
        sorted(
            root_items.items(),
            key=lambda item: (priority_index.get(item[0], len(priority)), item[0]),
        )
    )
